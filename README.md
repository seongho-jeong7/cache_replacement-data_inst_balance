...
- `scripts/run.sh`: ChampSim build/run/summary를 관리하는 메인 실행 스크립트. 기본 ChampSim 디렉터리는 `ChampSim_FDIP`이며, `-C`로 변경할 수 있다. `-f <mask>`로 FDIP FTQ 후보, `-L2C <mask>`로 L2C I/D partition 후보, `-w`/`-i`로 warmup/simulation instruction 수, `-r`로 run id, `-T`로 trace 목록 파일(`traces/` 기준)을 지정한다
- `outputs/`: 실험 결과 저장 위치. run id(`-r` 미지정 시 타임스탬프)별로 `outputs/<run id>/raw/fdip_<ftq>/<l2c_policy>/`에 trace 로그가 쌓이고, `scripts/run.sh -s <mask>` 실행 시 `outputs/<run id>/summary/fdip_<ftq>/<l2c_policy>/`에 metrics와 분석 결과가 생성된다
...
- `scripts/run.sh -f 0xff -L2C 0x1f -t`: 모든 FTQ 후보와 모든 L2C partition 후보를 실행한다.
- `scripts/run.sh -T trace_gtrace_l2c_test.txt -f 0x31 -L2C 0x1f -t`: L2C 실험용 trace 목록에서 `ftq0/32/64`와 모든 L2C partition을 실행한다.
- `scripts/run.sh -r <run_id> -s 0x41`: 지정한 run의 `metrics.csv`를 생성하고 summary table을 출력한다.
- `scripts/run.sh -r <run_id> -L2C 0x1f -s 0xC0`: L2C partition 비교용 metrics와 delta graph/csv를 생성한다.
