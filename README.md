...
- `scripts/run.sh`: 상단의 `CHAMPSIM_DIR` 안에 있는 `champsim_config.json`을 사용해 실행. `-w`/`-i`로 warmup/simulation instruction 수, `-r`로 run id, `-T`로 실행할 trace 목록 파일(`traces/` 기준)을 지정할 수 있다. `-t`는 trace만 실행하고, `-s <mask>`(1=summary table, 2=fdip cover, 4=hit map)를 실행해야 summary가 생성된다
- `outputs/`: 실험 결과 저장 위치. run id(`-r` 미지정 시 타임스탬프)별로 `outputs/<run id>/raw/fdip_<ftq>/`에 trace 로그가 쌓이고, `scripts/run.sh -s <mask>` 실행 시 `outputs/<run id>/summary/fdip_<ftq>/`에 metrics, FDIP cover, hit map이 생성된다
- `daily/`: 일별 연구 노트 작성 공간
- `docs/`: 연구 관련 문서 작성 공간
- `traces/`: DPC4/gtrace 계열 trace와 `trace_*.txt`(trace 목록 파일, `-T`로 지정) 위치
