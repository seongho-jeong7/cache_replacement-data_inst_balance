# 2026-07-21 코드 노트: ChampSim_L2C Split L2 구조

이 문서는 `ChampSim_L2C`에서 L2 cache를 실제 `L2I`와 `L2D` 하드웨어 구조로 분리하기 위해 반영한 코드 변경을 정리한다. 이전 `ChampSim_FDIP` 실험은 하나의 `L2C` 내부 way를 instruction/data로 나누는 방식이었다. 이번 변경의 목적은 config 단계에서 cache hierarchy 자체를 `L1I -> L2I -> LLC`, `L1D -> L2D -> LLC`로 구성할 수 있게 만드는 것이다.
...
이번 `ChampSim_L2C` 변경은 두 번째 방식이다. `run.sh`의 `-L2C` 옵션은 그대로 사용하되, `ChampSim_L2C/config/parse.py`가 `champsim_config.json`의 `L2C.partition`, `instruction_ways`, `data_ways`를 해석해서 shared 또는 split hierarchy를 만든다.
...
+    "partition", "instruction_ways", and "data_ways". For ChampSim_L2C, treat
...
`ChampSim_L2C`에는 FDIP/FTQ 구현이 없다. 하지만 root `scripts/run.sh`는 공통 실행 경로에서 `--ftq_size`를 넘긴다. 그래서 `src/main.cc`에서 옵션만 받고 무시하도록 했다.
...
의미는 명확하다. `-f 0x01`처럼 FTQ 옵션을 주더라도 `ChampSim_L2C`에서는 실험 축으로 사용하지 않는다.

## 빌드 관찰

`scripts/setup_champsim.sh -C ChampSim_L2C`로 dependency setup을 수행했다. 이후 `absolute.options`에 stale include path가 남아 있어서 vcpkg include 경로를 현재 `ChampSim_L2C/vcpkg_installed/x64-linux/include`로 맞춘 뒤 빌드가 진행됐다.
...
`260721_2005_w10_i100_l2c_split`에서 위 구조가 모두 확인됐다.
